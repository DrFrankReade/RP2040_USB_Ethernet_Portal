...
// Compatibility glue for the TinyUSB revision bundled in the pinned Pico SDK.
#ifndef tu_desc_subtype
#define tu_desc_subtype(desc) (((uint8_t const *)(desc))[2])
#endif

#ifndef NCM_NETWORK_CAPS_NTB_INPUT_SIZE
#define NCM_NETWORK_CAPS_NTB_INPUT_SIZE TU_BIT(5)
#endif
...
static bool xmit_insert_required_zlp(uint8_t rhport, uint32_t xferred_bytes) {
  ...
  TU_ASSERT(ncm_interface.itf_data_alt == 1);
  TU_ASSERT(!usbd_edpt_busy(rhport, ncm_interface.ep_in));
...
bool tud_network_can_xmit(uint16_t size) {
  ...
  TU_ASSERT(size <= ncm_interface.xmit_max_ntb_size - (sizeof(nth16_t) + sizeof(ndp16_t) + 2 * sizeof(ndp16_datagram_t)));
      ...
      switch (request->bRequest) {
        ...
        case NCM_SET_NTB_INPUT_SIZE: {
          if (stage == CONTROL_STAGE_SETUP) {
            ...
            TU_VERIFY(request->wLength == 4 || request->wLength == 8);
            if (request->wLength == 8) {
              TU_VERIFY(ncm_interface.bm_capabilities & NCM_NETWORK_CAPS_NTB_INPUT_SIZE);
            }
